...
- **`withOpenAiV1(url)`** strips trailing slashes + trailing `/v1`, then re-appends `/v1`. Applied to codex `base_url` and opencode `baseURL`; claude/gemini `base_url` use the plain stripped form (no `/v1`).
