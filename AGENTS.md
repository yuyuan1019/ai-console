...
A centralized web console + per-machine agent for managing AI coding CLI tools (Codex CLI, Claude Code, Gemini CLI, OpenCode, Pi, Hermes) across many remote Linux/macOS dev machines — configs, credentials, and model rollout from one dashboard, no SSH. Bilingual repo (Chinese UI, English code).

## Commands

There is **no JS/TS test runner, no lint script, and no monorepo workspace** in the Node apps — `console/package.json` is an empty `{}`, and installs are per-app with npm (lockfile is `package-lock.json`, not pnpm/yarn). Do not assume `npm test` / `npm run lint` exist. The Go agent does have a small test suite (`agent/internal/agent/agent_test.go`, run with `go test ./...`). Config-generation and crypto logic is untested — verify changes by hand.
...
node console/db/verify.cjs      # decrypt round-trip check; __dirname-relative, but needs a seeded DB
```
...
Browser ──HTTPS──▶ Console (Node Fastify API + React SPA, SQLite)
            ...
            WebSocket  │  (agent connects OUTBOUND — no inbound port needed on managed machines)
                       ▼
                 Agent (Go daemon) ── writes ──▶ ~/.codex, ~/.claude, ~/.gemini, ~/.config/opencode, ~/.pi/agent, ~/.hermes
```
...
- `console/db/` — `schema.sql` (reference snapshot + fallback) + `migrations/NNN_*.sql` (15 so far) + `seed.cjs`/`verify.cjs`/`migration-smoke.cjs`.
...
- `agent/internal/agent/agent.go` — the agent core, one ~1750-line file (plus `journal.go` ~150 lines and `agent_test.go`).
...
- **`set_credential`** writes a credential file the CLI sources. Codex → `~/.codex/auth.json` (`{"OPENAI_API_KEY":…}`). claude/gemini → `~/.ai-console-agent/creds/<tool>.sh` (shell exports), sourced from `.bashrc`/`.zshrc` via a loop the agent appends (`ensureCredSourcing`).
- **`write_config`** writes the tool's native config file (generated by `generateConfig`). Codex → `~/.codex/config.toml` (TOML); claude/gemini/opencode → JSON.
...
| claude | `~/.claude/settings.json` | `creds/claude.sh` (`ANTHROPIC_AUTH_TOKEN`, `ANTHROPIC_BASE_URL`) |
...
**Subscription account credentials are encrypted file snapshots, not API keys.** A user first runs `codex login` or `claude login` on a managed source machine, then `/api/servers/:id/account-credentials/import` creates a `read_account_credential` task. The agent reads `~/.codex/auth.json` or `~/.claude/.credentials.json` (Claude macOS Keychain fallback), deliberately skips the local task journal for this idempotent secret-bearing read, and the result handler immediately validates + encrypts the document into `provider_keys.encrypted_value` before stripping plaintext from `agent_tasks.result_json`. Auth types are `codex_subscription` and `claude_subscription`. The primary UI entry is the top-level Providers “新增” flow: choose OpenAI/Anthropic, then API Key or its subscription mode; failed source reads show the login command and a retry action. Delivery uses only `set_credential`, writes the native account file at `0600` with a `0600` backup, and deliberately skips provider config generation, API ping, and model refresh. Never persist the OAuth document in payload/audit/config plaintext.

**opencode is special-cased at every layer** and is the subject of the most recent commits. It accepts all providers (no family filter), supports multi-channel delivery (batch with >1 key merges N providers into one `opencode.json` via `mergeOpenCodeConfig`, with `config.model` set to the primary entry's `${providerId}/${model}`), previews a config-file path instead of env vars, and labels its button "下发配置" vs "下发订阅登录". Delivering opencode actually dispatches a `write_config` task — older code that sent `set_credential` for opencode silently delivered nothing (fixed in commits `e679877`/`cb50059`/`2373c38`). Treat opencode paths as recently-churned.
...
- The `baseUrl` is normalized with `withOpenAiV1` (trailing `/v1`) — but only for `openai-completions`/`openai-responses`; `anthropic-messages`/`google-generative-ai` keep the raw baseUrl (the client appends `/v1/messages` / `/v1beta/...` itself).
...
React 19 + Vite + Tailwind 3 + shadcn-style UI (`components/ui/*`). State is split: server data via `@tanstack/react-query` (hooks in `src/hooks/*`), local UI via `useState` in pages, and zustand is used only for `lib/theme.ts` (dark mode; not persisted — reload always returns to dark) plus a tiny toast store in `lib/toast.tsx`. `lib/api.ts` is a hand-rolled `fetch` wrapper (not axios), base hardcoded to `/api`, access token in `localStorage` (`ai_console_access_token`) sent as `Bearer`, `credentials:'include'` for the refresh cookie. `lib/ws.ts` connects to `/api/ws?ticket=…` (browsers can't set WS headers) where the ticket is a one-shot 30s value from `POST /api/auth/ws-ticket` — the access JWT never goes into the URL — with 1s→1.5x backoff capped 30s; hooks `subscribe(channel, cb)` where `cb` invalidates react-query keys. **Most WS-backed queries also set `refetchInterval:10000` as a polling fallback** (exception: `useProviders` relies on WS only). Channels: `servers:status`, `server:<id>:tasks`, `batch:<id>`, `provider:<id>:keys`. Query keys are stable tuples (e.g. `['server', id, 'tasks']`).
...
- **Global rate limiting is off, but login opts in** — `@fastify/rate-limit` is registered with `global:false, max:0`; `/api/auth/login` opts in at 5 req / 15 min per `ip|username`, and 5 consecutive failures lock the account for 5 min. Enroll and other routes are not throttled.
...
- **`withOpenAiV1(url)`** strips trailing slashes, keeps an existing version segment (`/v1`–`/v4`, e.g. GLM `/api/coding/paas/v4`, Ark `/api/v3`) untouched, and appends `/v1` only when the URL has none. Applied to codex `base_url` and opencode `baseURL`; claude/gemini `base_url` use the plain stripped form (no `/v1`).
- **`seed.cjs`/`verify.cjs` are fragile:** both require `seed/cc-switch-import.json` (not in repo, gitignored) and exit on ENOENT; `verify.cjs` additionally refuses to run unless `console/data/ai-console.db` already exists (it never creates one).
- **Agent-side file permissions are private (`0600`/`0700`)** — `~/.ai-console-agent` and its `creds/` are `0700`; config files, backups, `state.json`, `task-journal.json`, `creds/*.sh`, `~/.codex/auth.json` and `~/.claude/.credentials.json` are written atomically at `0600` (`enforceOwnedPathPermissions` re-checks on startup).
- **Tool lifecycle runs on the managed host.** npm-based tools use their allowlisted global package (`npm install/uninstall --global`). For OpenCode, the agent resolves the command symlink first: npm installs (including `~/.local/bin/opencode` symlinks) upgrade through npm, while curl/bun/pnpm/brew installs use native `opencode upgrade --method ...`; this avoids OpenCode misclassifying every `~/.local/bin` command as curl. Hermes uses its official installer/native lifecycle commands. The agent verifies the reported target version after install/upgrade instead of trusting a zero exit status.
