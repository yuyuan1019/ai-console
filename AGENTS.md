...
- **Tool lifecycle runs on the managed host.** npm-based tools use their allowlisted global package (`npm install/uninstall --global`); OpenCode upgrades through its native `opencode upgrade` command so curl/bun/pnpm/brew installs do not get shadowed by a second npm copy; Hermes uses its official installer/native lifecycle commands. The agent verifies the reported target version after install/upgrade instead of trusting a zero exit status.
